...
public static class ValueEquality
{
	...
	private static readonly Type[] objecttypearray = [typeof(object)];
	...
	public static void CheckAssembly(params Type[] ignoretypes)
	{
		...
		if (runflag) {
			throw new InvalidOperationException("ValueEquality.CheckAssembly() has already been run");
		}
	...
	private static bool HasEqualsMethod(Type type)
	{
		...
		if (equalsMethodObj?.DeclaringType == type) {
			return true;
		}
		...
		foreach (var method in methods) {
			...
			if (method.Name == "Equals") {
				...
				if (type.IsValueType && paramtype.IsValueType) {
					...
					if (paramtype != type && !nullableType.IsAssignableFrom(paramtype)) {
						continue;
					}
				} else {
